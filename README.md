...
| 功能 | 说明 |
|------|------|
| 首页 | 首次进入先弹出授权确认框；用户同意授权后获取头像昵称，可选输入自定义昵称，确认后才可创建房间或加入房间；视觉已升级为深墨绿色丝绒桌布风格 |
| 房间页 | 房间号；玩家按入房顺序稳定排座且本人固定最下；头像外圈显示状态；只显示自己手牌与公牌；准备 / 发牌 / 开牌；邀请好友（转发）；桌面中心强化公牌焦点 |
| 结果页 | 所有玩家手牌 + 公牌；任意玩家开牌后全员同步进入结果页；返回房间后等待再次准备；沿用牌桌丝绒视觉和更强的公牌区层次 |
...
- [x] 房间页视觉与座位规则升级：其他玩家基于入房顺序稳定围桌展示，当前玩家固定底部，公牌区增强为牌桌中心焦点。
- [x] 扑克牌样式升级：房间页与结果页牌面字号放大，`♥` / `♦` 显示为红色，公牌和手牌统一采用更明显的卡片视觉。
- [x] 三页主题统一：首页 / 房间页 / 结果页都已切换为深墨绿丝绒牌桌质感，按钮同步升级为更贴近赌场桌面的风格。
...
- [ ] 在多机联调时继续验证 5 人以上房间的座位分布与昵称换行表现。
...
- `main`
  - 正式版分支，只保留可上线、可预览、可提交审核的功能与 UI。
- `test/mock-multiplayer`
  - 测试版分支，保留模拟多人调试能力，包括 `mockRoomAction` 云函数与房间页测试面板。
  - 若要验证单微信号下的多人房间展示，可切到该分支进行测试。
...
- **房间页围桌布局已稳定**：当前玩家固定底部，其他玩家基于入房顺序沿逆时针方向分布到左右两侧，保证所有客户端相对顺序一致。
- **视觉主题已升级为丝绒牌桌风格**：三页背景已统一为深墨绿桌布质感；公牌区、玩家区域和底部按钮都增强了桌面层次和游戏感。
