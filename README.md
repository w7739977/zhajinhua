...
10. **正式版与测试版要分支隔离**  
   模拟玩家、测试面板、测试云函数这类能力很适合放在独立测试分支中验证，不要直接混入正式版。  
   **当前策略**：`main` 只保留正式可发布内容；`test/mock-multiplayer` 保留 `mockRoomAction`、模拟玩家面板与多人测试工具。

11. **多人座位展示要基于“稳定顺序 + 本人置底”**  
   房间页不是简单按数组平铺。更自然的做法是：以入房顺序为基础，围绕当前玩家做一次旋转，让自己始终固定在底部，然后把其他玩家稳定拆到左右两侧。这样每个客户端看到的相对座位关系一致，同时又符合“我在下方”的游戏习惯。
